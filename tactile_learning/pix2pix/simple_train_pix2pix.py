...
def main(opt, augmentation_params, weights, task_dirs, data_dirs):
    ...
    image_size_str = (
        str(augmentation_params["dims"][0])
        + "x"
        + str(augmentation_params["dims"][1])
    )
    ...
    training_real_data_dirs = [
        os.path.join(
            cur_file, "../data_collection/real/data/", data_path, "csv_train"
        )
        for data_path in combined_paths
    ]
    validation_real_data_dirs = [
        os.path.join(
            cur_file, "../data_collection/real/data/", data_path, "csv_val"
        )
        for data_path in combined_paths
    ]
    training_sim_data_dirs = [
        os.path.join(
            cur_file,
            "../data_collection/sim/data/",
            data_path,
            image_size_str,
            "csv_train",
        )
    ...
    save_dir_name = os.path.join(
        "saved_models",
        task_str,
        image_size_str + "_" + dir_str + "_" + str(opt.n_epochs) + "epochs",
    )
    ...
    # save params
    save_json_obj(
        augmentation_params, os.path.join(save_dir_name, "augmentation_params")
    )
    ...
    patch = (
        1,
        augmentation_params["dims"][0] // 2 ** 4,
        augmentation_params["dims"][1] // 2 ** 4,
    )

    # Initialize generator and discriminator
    generator = GeneratorUNet(
        in_channels=opt.channels, out_channels=opt.channels
    )
    ...
    if opt.epoch != 0:
        ...
        generator.load_state_dict(
            torch.load(os.path.join(checkpoint_dir, "final_generator.pth"))
        )
        discriminator.load_state_dict(
            torch.load(
                os.path.join(checkpoint_dir, "final_discriminator_{}.pth")
            )
        )
    else:
        # Initialize weights
    ...
    training_generator = DataGenerator(
        real_data_dirs=training_real_data_dirs,
    ...
    val_generator = DataGenerator(
        real_data_dirs=validation_real_data_dirs,
    ...
    def sample_images(batches_done):
        ...
        img_sample = torch.cat(
            (
                real_imgs.data[:n_save_images, :, :, :],
                gen_sim_imgs.data[:n_save_images, :, :, :],
                sim_imgs.data[:n_save_images, :, :, :],
            ),
            -2,
        )
        save_image(
            img_sample,
            os.path.join(image_dir, "{}.png".format(batches_done)),
            nrow=4,
            normalize=False,
        )
    ...
    for epoch in range(opt.epoch, opt.n_epochs + 1):
        for i, batch in enumerate(training_loader):
            ...
            valid = Variable(
                Tensor(np.ones((tip_images.size(0), *patch))),
                requires_grad=False,
            )
            fake = Variable(
                Tensor(np.zeros((tip_images.size(0), *patch))),
                requires_grad=False,
            )
            ...
            # Total loss
            loss_G = (weights["W_gan"] * loss_GAN) + (
                weights["W_pixel"] * loss_pixel
            )
        ...
        if (epoch % opt.sample_interval == 0) or ((epoch) % opt.n_epochs == 0):
            ...
            torch.save(
                ...
                os.path.join(checkpoint_dir, "final_generator.pth"),
            )
            torch.save(
                discriminator.state_dict(),
                os.path.join(checkpoint_dir, "final_discriminator.pth"),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_epochs", type=int, default=250, help="number of epochs of training"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="shuffle the generated image data",
    )
    parser.add_argument(
        "--sample_interval",
        type=int,
    ...
    if list(augmentation_params["dims"]) == [256, 256]:
        ...
        from tactile_gym_sim2real.pix2pix.gan_models.models_128 import (
            GeneratorUNet,
            Discriminator,
            weights_init_normal,
        )
